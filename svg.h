...
//Методы возвращают ссылку на сам объект, для облегчения поочерёдного их вызова.
...
protected:
    ...
    void RenderAttrs(std::ostream& out) const {
        ...
        if (line_cap_) {
            out << " stroke-line_cap_=\""sv << *line_cap_ << "\""sv;
        }
        if (line_join_) {
            out << " stroke-line_join_=\""sv << *line_join_ << "\""sv;
        }
